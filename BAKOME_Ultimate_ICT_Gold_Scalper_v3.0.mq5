...
public:
   ...
   void Release(T* obj) {
      ...
      obj.Reset();
      for(int i = 0; i < m_nextAvailable; i++) {
         if(m_pool[i] == obj) {
            if(i != m_nextAvailable - 1) {
               T* temp = m_pool[i];
               m_pool[i] = m_pool[m_nextAvailable - 1];
               m_pool[m_nextAvailable - 1] = temp;
            }
            m_nextAvailable--;
            break;
         }
...
private:
   ...
   double RiskLot(double riskPercent, double atr) {
      ...
      if(tickSize > 0 && tickValue > 0)
         ...
         lossPerLot = slDistance * 100.0; // Fallback for standard gold contract
   ...
   void ApplyBreakEven(CPositionTracker &pos) {
      ...
      if(m_position.SelectByTicket(pos.ticket)) {
         ...
         double openPrice = m_position.OpenPrice();
         double beTriggerDist = m_currentATR * BE_TriggerATR;
         bool trig = false;

         if(m_position.PositionType() == POSITION_TYPE_BUY) {
            double currentProfitDist = m_symbol.Bid() - openPrice;
            if(currentProfitDist >= beTriggerDist) trig = true;
         } else if(m_position.PositionType() == POSITION_TYPE_SELL) {
            double currentProfitDist = openPrice - m_symbol.Ask();
            if(currentProfitDist >= beTriggerDist) trig = true;
         }

         if(trig) {
            double newSL = NormalizeDouble(openPrice, (int)m_symbol.Digits());
            if(m_trade.PositionModify(pos.ticket, newSL, m_position.TakeProfit())) {
               pos.breakEvenSet = true;
               LogInfo(StringFormat("BreakEven applied to ticket %d @ %.2f", pos.ticket, newSL));
            }
         }
      }
   }
   ...
   void ManageTrailingStop(CPositionTracker &pos) {
      ...
      if(m_position.SelectByTicket(pos.ticket)) {
         m_symbol.RefreshRates();
         double openPrice = m_position.OpenPrice();
         double trailStartDist = m_currentATR * Trail_StartATR;
         double trailStepDist = m_currentATR * Trail_StepATR;
         int digits = (int)m_symbol.Digits();

         if(m_position.PositionType() == POSITION_TYPE_BUY) {
            double currentProfitDist = m_symbol.Bid() - openPrice;
            if(currentProfitDist >= trailStartDist) {
               double newSL = NormalizeDouble(m_symbol.Bid() - trailStepDist, digits);
            ...
            double currentProfitDist = openPrice - m_symbol.Ask();
            if(currentProfitDist >= trailStartDist) {
               double newSL = NormalizeDouble(m_symbol.Ask() + trailStepDist, digits);
               double currentSL = m_position.StopLoss();
   ...
   bool HasBullishConfluence() {
      bool sweepOk = !UseLiquiditySweeps;
      bool fvgOk   = !UseFairValueGaps;
      bool obOk    = !UseOrderBlocks;
      ...
      if(UseLiquiditySweeps) {
         for(int i = 0; i < ArraySize(m_liquidityLevels); i++) {
            if(!m_liquidityLevels[i].isHigh && (bid <= m_liquidityLevels[i].price || iLow(_Symbol, PERIOD_M5, 0) <= m_liquidityLevels[i].price)) {
               sweepOk = true;
               break;
            }
         }
      ...
      if(UseFairValueGaps) {
         for(int i = 0; i < ArraySize(m_fairValueGaps); i++) {
            if(m_fairValueGaps[i].isBullish && !m_fairValueGaps[i].filled) {
               if(ask >= m_fairValueGaps[i].bottomPrice && ask <= m_fairValueGaps[i].topPrice) {
                  fvgOk = true;
                  break;
               }
            }
      ...
      if(UseOrderBlocks) {
         for(int i = 0; i < ArraySize(m_orderBlocks); i++) {
            if(m_orderBlocks[i].isBullish && !m_orderBlocks[i].mitigated) {
               if(ask >= m_orderBlocks[i].bottomPrice && ask <= m_orderBlocks[i].topPrice) {
                  obOk = true;
                  break;
               }
            }
   ...
   bool HasBearishConfluence() {
      ...
      if(UseLiquiditySweeps) {
         for(int i = 0; i < ArraySize(m_liquidityLevels); i++) {
            if(m_liquidityLevels[i].isHigh && (ask >= m_liquidityLevels[i].price || iHigh(_Symbol, PERIOD_M5, 0) >= m_liquidityLevels[i].price)) {
               sweepOk = true;
               break;
            }
         }
      ...
      if(UseFairValueGaps) {
         for(int i = 0; i < ArraySize(m_fairValueGaps); i++) {
            if(!m_fairValueGaps[i].isBullish && !m_fairValueGaps[i].filled) {
               if(bid >= m_fairValueGaps[i].bottomPrice && bid <= m_fairValueGaps[i].topPrice) {
                  fvgOk = true;
                  break;
               }
            }
      ...
      if(UseOrderBlocks) {
         for(int i = 0; i < ArraySize(m_orderBlocks); i++) {
            if(!m_orderBlocks[i].isBullish && !m_orderBlocks[i].mitigated) {
               if(bid >= m_orderBlocks[i].bottomPrice && bid <= m_orderBlocks[i].topPrice) {
                  obOk = true;
                  break;
               }
            }
   ...
   void ExecuteTrade(int type) {
      ...
      if(m_lastTradeTime == currentBar) return;
      ...
      if(ExecuteWithRetry(request, result)) {
         ...
         m_todayTradeCount++;
         m_lastTradeTime = currentBar;
         CPositionTracker* pos = m_positionPool.Acquire();
         pos.ticket = (result.position > 0) ? result.position : result.order;
...
public:
   ...
   bool Init() {
      ...
      m_lastBarTime = 0;
      m_lastTradeTime = 0;
      m_lastDayTime = iTime(_Symbol, PERIOD_D1, 0);
      m_initialized = true;
   ...
   void OnTick() {
      ...
      if(currentDay != m_lastDayTime) {
         m_lastDayTime = currentDay;
         ResetDailyStats();
         LogInfo("Daily statistics reset for new trading day.");
      }

      datetime currentBarTime = iTime(_Symbol, PERIOD_M5, 0);
      if(!EnablePerformanceMode || currentBarTime != m_lastBarTime) {
      ...
      if(IsInKillZone()) {
         ...
         if(bias == POSITION_TYPE_BUY && HasBullishConfluence())
            ExecuteTrade(ORDER_TYPE_BUY);
         else if(bias == POSITION_TYPE_SELL && HasBearishConfluence())
